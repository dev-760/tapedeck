...
export default function WalkmanHomeListeningDeck() {
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }}>
          ...
          <View style={s.deckCard}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 4, marginBottom: 8 }}>
              <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                <View style={s.engagedBadge}><Text style={s.engagedTxt}>ENGAGED</Text></View>
                <Text style={s.deckLabel}>DECK A // CHASSIS WELL</Text>
              </View>
              <View style={s.counterBox}><Text style={s.counterTxt}>0234</Text></View>
            </View>
            ...
            <View style={s.wellFrame}>
              <View style={s.shellBody}>
                ...
                <View style={s.labelCard}>
                  <View style={{ flex: 1 }}>
                    <Text style={s.labelTitle} numberOfLines={1}>STARMAN — DAVID BOWIE</Text>
                    <Text style={s.labelSub} numberOfLines={1}>The Rise and Fall of Ziggy Stardust (1972)</Text>
                  </View>
                  <View style={s.durationBadge}><Text style={s.durationTxt}>4:16</Text></View>
                </View>
                ...
                <View style={s.spoolWindow}>
                  ...
                  <View style={s.spoolLarge}>
                    <View style={s.tapePackLarge}>
                      <View style={s.spoolHub}><MaterialIcons name="settings" size={14} color={COLORS.onPrimary} /></View>
                    </View>
                  </View>
                  <View style={{ alignItems: 'center', gap: 4, zIndex: 10 }}>
                    <Text style={s.spoolTime}>02:18</Text>
                    <View style={s.progressBar}><View style={[s.progressFill, { width: '55%' }]} /></View>
                    <Text style={s.runningTxt}>RUNNING</Text>
                  </View>
                  <View style={s.spoolLarge}>
                    <View style={s.tapePackSmall}>
                      <View style={s.spoolHub}><MaterialIcons name="settings" size={14} color={COLORS.onPrimary} /></View>
                    </View>
                  </View>
                </View>
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 16 }}>
              {[
                { i: 'fast-rewind', l: 'REWIND' },
                { i: 'pause', l: 'PAUSE' },
                { i: 'play-arrow', l: 'PLAYING', active: true },
                { i: 'eject', l: 'EJECT' },
              ].map((btn, i) => (
                <TouchableOpacity key={i} style={[s.ctrlBtn, btn.active && { backgroundColor: COLORS.secondaryContainer }]}>
                  <MaterialIcons name={btn.i as any} size={18} color={btn.active ? COLORS.secondary : COLORS.onSurface} />
                  <Text style={[s.ctrlBtnTxt, btn.active && { color: COLORS.secondary }]}>{btn.l}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 }}>
              ...
              <TouchableOpacity style={s.openBtn}><Text style={s.openBtnTxt}>OPEN TRANSPORT</Text><MaterialIcons name="north-east" size={14} color={COLORS.onSecondary} /></TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View style={{ marginTop: 24 }}>
          <View style={{ paddingHorizontal: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
              <MaterialIcons name="shelves" size={16} color={COLORS.secondary} />
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, gap: 8 }}>
            {[
              { title: '01. Karma Police', artist: 'Radiohead — OK Computer', type: 'SIDE A • CR-O2', color: COLORS.errorContainer },
              { title: '02. Nightcall', artist: 'Kavinsky — OutRun Synth', type: 'SIDE A • FERRIC', color: COLORS.tertiaryContainer },
              { title: '03. Blue in Green', artist: 'Miles Davis — Kind of Blue', type: 'SIDE B • METAL IV', color: COLORS.outlineVariant },
            ].map((t, i) => (
              <TouchableOpacity key={i} style={s.spineCard}>
                ...
                <TouchableOpacity style={s.loadIconBtn}><MaterialIcons name="input" size={18} color={COLORS.onSurface} /></TouchableOpacity>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 24 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
              <MaterialIcons name="album" size={16} color={COLORS.secondary} />
          ...
          <View style={s.mixtapeCard}>
            ...
            <View style={s.mixtapeContent}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 }}>
                ...
                <TouchableOpacity style={s.loadAllBtn}><MaterialIcons name="play-circle" size={16} color={COLORS.onPrimary} /><Text style={s.loadAllTxt}>LOAD ALL 12 TRACKS</Text></TouchableOpacity>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 24 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
              ...
              <Text style={s.sectionTitle}>RECENT ARCHIVE ACCESS</Text>
            </View>
            <Text style={s.loggedTxt}>LOGGED 24H</Text>
          </View>
          <View style={{ flexDirection: 'row', gap: 8 }}>
            {[
              { title: 'Unknown Pleasures', artist: 'Joy Division • Side B', type: 'TYPE I FERRIC', time: '32m', last: 'LAST PLAYED 2H AGO' },
              { title: 'Random Access', artist: 'Daft Punk • Side A', type: 'TYPE II CrO2', time: '44m', last: 'LAST PLAYED YESTERDAY' },
            ].map((r, i) => (
              <TouchableOpacity key={i} style={s.recentCard}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  <View style={s.typeBadge}><Text style={s.typeBadgeTxt}>{r.type}</Text></View>
                  <Text style={s.recentTime}>{r.time}</Text>
                </View>
                <Text style={s.recentTitle}>{r.title}</Text>
                <Text style={s.recentArtist}>{r.artist}</Text>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                  <Text style={s.recentLast}>{r.last}</Text>
      ...
      <View style={s.bottomPlayer}>
        <View style={s.bpHub}><MaterialIcons name="settings" size={16} color={COLORS.secondary} /></View>
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
            <View style={s.miniSideBadge}><Text style={s.miniSideTxt}>SIDE A</Text></View>
            <Text style={s.bpTime}>02:18</Text>
          </View>
          <Text style={s.bpTitle} numberOfLines={1}>Starman — David Bowie</Text>
        </View>
        <TouchableOpacity style={s.bpBtn}><MaterialIcons name="fast-rewind" size={16} color={COLORS.onSurface} /></TouchableOpacity>
        <TouchableOpacity style={[s.bpBtn, { backgroundColor: COLORS.secondary }]}><MaterialIcons name="pause" size={18} color={COLORS.onSecondary} /></TouchableOpacity>
        <TouchableOpacity style={s.bpBtn}><MaterialIcons name="fast-forward" size={16} color={COLORS.onSurface} /></TouchableOpacity>
      </View>

      {/* Bottom Nav */}
      <View style={s.navBar}>
        {[{ i: 'home', l: 'WALKMAN', active: true }, { i: 'shelves', l: 'SHELF' }, { i: 'search', l: 'LOCATE' }, { i: 'album', l: 'MIXTAPES' }].map((n, i) => (
...
const s = StyleSheet.create({
  ...
  tapePackSmall: { width: 44, height: 44, borderRadius: 22, backgroundColor: COLORS.secondaryContainer, alignItems: 'center', justifyContent: 'center' },
  ...
  loggedTxt: { fontFamily: FONTS.displayBold, fontSize: 10, color: COLORS.primary, textTransform: 'uppercase' },
  recentCard: { flex: 1, backgroundColor: COLORS.surfaceContainer, padding: 12, borderRadius: 4 },
  typeBadge: { backgroundColor: COLORS.surfaceContainerHighest, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 2 },
  typeBadgeTxt: { fontFamily: FONTS.displayBold, fontSize: 9, color: COLORS.secondary, textTransform: 'uppercase' },
  recentTime: { fontFamily: FONTS.monoBold, fontSize: 14, color: COLORS.onSurfaceVariant },
  recentTitle: { fontFamily: FONTS.monoBold, fontSize: 14, color: COLORS.onSurface, marginTop: 8 },
  recentArtist: { fontFamily: FONTS.mono, fontSize: 11, color: COLORS.onSurfaceVariant },
  recentLast: { fontFamily: FONTS.displayBold, fontSize: 8, color: COLORS.primary, textTransform: 'uppercase', letterSpacing: 1 },
  bottomPlayer: { position: 'absolute', bottom: 56, left: 16, right: 16, backgroundColor: 'rgba(40,42,44,0.95)', padding: 8, borderRadius: 8, flexDirection: 'row', alignItems: 'center', gap: 8, shadowColor: '#000', shadowOpacity: 0.8, shadowRadius: 8, elevation: 8 },
  ...
  navLabel: { fontFamily: FONTS.displayBold, fontSize: 9, color: COLORS.onSurfaceVariant, textTransform: 'uppercase', letterSpacing: 1 },
});